...
def get_entire_stream_rainfall_list(*dates):
    entire_list = [["obs_id", "updated_at", "rainfall"]]
    ...
    for date in convert_dates:
        for id in row:
            ...
            for element in elements:
                entire_list.append([element["rfobscd"].strip(), element["ymdhm"].strip(), element["rf"]])
...
task6_2 = PythonOperator(
    ...
    params = {
        ...
        "s3_key": f"""rainfall/{tables_info[1]["table_name"]}_202407.csv""",
        "date": "202407"
    },
    dag = dag
)
...
task7_1 = S3ToRedshiftOperator(
    ...
    schema = schema,
    table = tables_info[0]["table_name"],
    column_list = ["obs_id","obs_name","lat","lon","gov_agency","addr","etc_addr","rel_river","opened_at","first_at","last_at"],
