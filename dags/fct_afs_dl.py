...
def fct_afs_dl_to_s3(**kwargs: Any) -> str:
    ...
    api_df["TM_FC"] = pd.to_datetime(api_df["TM_FC"], format="%Y%m%d%H%M")
    api_df["TM_EF"] = pd.to_datetime(api_df["TM_EF"], format="%Y%m%d%H%M")
...
def load_to_redshift(**kwargs: Any) -> None:
    ...
    # 새로운 컬럼 추가
    api_df["DATA_KEY"] = pd.to_datetime(current_time, format="%Y%m%d%H", errors="coerce")
    api_df["CREATE_AT"] = api_df["TM_FC"]
    api_df["UPDATE_AT"] = api_df["TM_FC"]
    logging.info("데이터프레임 필수 컬럼 추가:\n%s", api_df.head())

    api_df_filtered = api_df.drop(columns=["STN", "C", "MAN_ID", "MAN_FC"])
    ...
    try:
        ...
        conn.commit()
        logging.info("데이터 적재 성공 'FCT_AFS_DL_INFO'.")
    except Exception as e:
        conn.rollback()
